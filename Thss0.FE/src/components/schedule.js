...
const Schedule = props => {
    ...
    const path = `users/professional/true/20/1`// Test.
    const timeLinePts = 97
    UseUpdate(props, path)
    if (!(props.content && props.content[0]['procedures'])) {
        return
    }
    let sourceCellIndex
    ...
    return <>
        ...
        <table className="table">
            ...
            <tbody onMouseDown={event => selectCell(event)} onMouseOver={event => hoverCell(event)} onMouseUp={() => sourceCellIndex = null}>
...
const mapDispatchToProps = dispatch => {
    return {
        updateContent: async (stateCopy, path) => {
            ...
            if (procByProf) {
                dispatch(updateContent(procByProf, stateCopy.totalPages, stateCopy.localOrder, stateCopy.currentPage))
            }
        }
    }
}
